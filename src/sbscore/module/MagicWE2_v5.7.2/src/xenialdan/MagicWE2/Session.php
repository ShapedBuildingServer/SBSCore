...
class Session{
	...
	private $selections = [];
	/** @var int */
	private $quickReplace = [];
	/** @var UUID|null */
	private $latestselection = null;
	/** @var UUID|null */
	private $latestQuickReplace = null;
	/** @var Clipboard[] */
	private $clipboards = [];
	/** @var Clipboard[] */
	private $undo = [];
	...
	public function &addQuickReplace(QuickReplace $replace){
		$this->replace[$replace->getUUID()->toString()] = $replace;
		$this->setLatestQuickReplaceUUID($replace->getUUID());
		$this->latestQuickReplace = $replace->getUUID();
		return $replace;
	}
	 ...
	 * @return null|Selection
	 */
	public function &getLatestQuickReplace(){
		$latestQuickReplace = $this->getLatestQuickReplaceUUID();
		if (is_null($latestQuickReplace)){
			$replace = null;
			return $replace;
		}
		$replace = $this->replace[$latestQuickReplace->toString()] ?? null;
		return $replace;
	}
	 ...
	 * @return UUID|null
	 */
	public function getLatestQuickReplaceUUID(): ?UUID{
		return $this->latestQuickReplace;
	}
	...
	/**
	 * @param UUID $latestQuickReplace
	 */
	public function setLatestQuickReplaceUUID(UUID $latestQuickReplace){
		$this->latestQuickReplace = $latestQuickReplace;
	}
