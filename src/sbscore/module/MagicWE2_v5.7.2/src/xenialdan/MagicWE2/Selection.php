...
class Selection{
	...
	public function setPos1(Position $position){
		$this->pos1 = $position;
		return Loader::$prefix . "Position 1 set to X: " . $position->getX() . " Y: " . $position->getY() . " Z: " . $position->getZ();
	}
	...
	public function setPos2(Position $position){
		$this->pos2 = $position;
		return Loader::$prefix . "Position 2 set to X: " . $position->getX() . " Y: " . $position->getY() . " Z: " . $position->getZ();
	}
				...
				for ($z = floor($this->getAxisAlignedBB()->minZ); $z <= floor($this->getAxisAlignedBB()->maxZ); $z++){
					...
					if (empty($filterblocks)) $blocks[] = $block;
						...
						foreach ($filterblocks as $filterblock){
							...
							&& ((API::hasFlag($flags, API::FLAG_VARIANT) && $block->getVariant() === $filterblock->getVariant())
							|| (!API::hasFlag($flags, API::FLAG_VARIANT) && ($block->getDamage() === $filterblock->getDamage() || API::hasFlag($flags, API::FLAG_KEEP_META)))))
								$blocks[] = $block;
						}
					}
				}
