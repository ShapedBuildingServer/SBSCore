...
class API{
	...
	public static function setSingle(QuickReplace $replace, ?Session $session, Position $block){
		$level = $block->getLevel() ?? (!is_null($session) ? $session->getPlayer()->getLevel() : null);
		$level->setBlock($block, $replace->getBlock());
	}
	...
	public static function pasteSchematic(Session $session, $newblocks = [], ...$flagarray){
		$flags = self::flagParser($flagarray);
	...
	/**
	 * Funciton taken from BlockSniper
	 * https://github.com/BlockHorizons/BlockSniper/blob/194ee1de9294d27e5219cdfc2b6d8ea2cc337003/src/BlockHorizons/BlockSniper/listeners/BrushListener.php#L126
	 * @param Player $player
	 *
	 * @return Vector2[][]
	 */
	public static function getPlotPoints(Player $player) : array{
		if(!Loader::isMyPlotAvailable()){
			return [];
		}
		$plotPoints = [];
		$settings = Loader::getMyPlot()->getLevelSettings($player->getLevel()->getName());
		if($settings === null){
			if($player->hasPermission("we.myplot-allow-outside")){
				return [];
			}
			return [[new Vector2(), new Vector2()]];
		}
		$plotSize = $settings->plotSize;
		foreach(Loader::getMyPlot()->getPlotsOfPlayer($player->getName(), $player->getLevel()->getFolderName()) as $plot){
			$minVec = new Vector2(self::calcActual($plot->X, $settings) - $plotSize, self::calcActual($plot->Z, $settings) - $plotSize);
			$maxVec = new Vector2(self::calcActual($plot->X, $settings) - 1, self::calcActual($plot->Z, $settings) - 1);
	...
	private static function calcActual(int $coordinate, PlotLevelSettings $settings) : int{
		$coordinate += 1;
		return $coordinate * $settings->plotSize + ($coordinate - 1) * $settings->roadWidth;
	}

	public static function isPointInPlayerPlot($point, Player $player) :bool
	{
		if (Loader::getMyPlot()->getLevelSettings($player->getLevel()->getName()) === null) return true;

		foreach (self::getPlotPoints($player) as $points) {
			$first = $points[0];
			$second = $points[1];
			if ($point->x >= $first->x && $point->x <= $second->x
			&& $point->z >= $first->y && $point->z <= $second->y) {
				return true;
			}
		}
		return false;
