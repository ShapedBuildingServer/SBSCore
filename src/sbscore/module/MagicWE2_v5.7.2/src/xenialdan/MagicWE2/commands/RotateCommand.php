...
class RotateCommand extends WECommand{
	public function __construct(Plugin $plugin){
		...
		$this->setUsage("//rotate <degrees X>");
	}
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		try{
			...
			if ($args[0] % 90 !== 0) {
				$sender->sendMessage("Invalid degrees.");
				return;
			}

			$sender->sendMessage(Loader::$prefix . "Trying to rotate clipboard by " . $args[0] . " degrees");
